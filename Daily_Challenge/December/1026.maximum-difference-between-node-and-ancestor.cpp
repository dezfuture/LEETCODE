...
private:
    void solve(TreeNode *root, int maxn, int minn)
    {
        if (!root)
        {
            return;
        ...
        res = max({res, root->val - minn, maxn - root->val});
        minn = min(root->val, minn);
        maxn = max(maxn, root->val);

        // dfs recursion for the whole tree
        solve(root->left, maxn, minn);
        solve(root->right, maxn, minn);
    }
};
// @lc code=end
