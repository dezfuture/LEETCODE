...
public:
    bool canReach(vector<int> &arr, int start)
    {
        ...
        return solve(arr, start, i, j, v);
    }

private:
    bool solve(vector<int> &arr, int curr, int low, int high, vector<bool> &v)
    {
        if (curr < low || curr > high || v[curr])
        {
            return false;
        }
        if (arr[curr] == 0)
        ...
        return solve(arr, curr + arr[curr], low, high, v) + solve(arr, curr - arr[curr], low, high, v);
    }
};
// @lc code=end
